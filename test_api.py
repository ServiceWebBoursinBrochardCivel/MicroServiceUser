...
def test_Exist():
    r = requests.post("http://127.0.0.1:5001/login", json={"mail": "test@test.com","password": "test"})
    resultAtt = 200
    assert r.text == resultAtt


def test_NotExist():
    r = requests.post("http://127.0.0.1:5001/login", json={"mail": "INCORECT@FAUX.com","password": "INCORRECT"})
    resultAtt = "Can't authenticate", 401
    assert r.text == resultAtt
